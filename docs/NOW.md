...
**Vadim nod batch — REVIEWED 2026-07-31 (Vadim's lane, per HANDOFF-VADIM §6; merging that PR records the nod):** ✅ shared-schema cap fields (`DecisionConfig.maxTicketUsdc`/`dailyCapRemainingUsdc` — optional/additive in `packages/shared/src/decision.ts`, engine clamps deployable via `bmin` at `engine.ts:157`; backwards-compatible) · ✅ venue seam (`IVenue`, `agent/src/chain/usyc-venue.ts` — strictly read-only, money moves exist only as call specs the ChainExecutor signs; allowlist check correctly reads `RolesAuthority.canCall`) · ✅ mandate-interface generalization (AgentMandateV2 keeps the six-event surface byte-for-byte; verifier `--address` verifies both — exercised by the venue-aware verifier work) · ✅ AgentMandate + trade-gate sims (gates a/b/c/g in `trade-gate.sim.test.ts`, green in the 115-test agent suite; cap-exhaustion produces HOLD/SKIPPED, never a revert storm).
