...
- [x] **Golden snapshot refreshed 2026-07-27 (4 → 7 moves)** — and made repeatable: `npm run snapshot -w verifier` (`verifier/scripts/snapshot.ts`) rescans live history, refuses to write a non-COMPLIANT snapshot, and prints the expectations to paste into `golden.test.ts` (wiring stays a human edit — a self-updating golden test can't fail). Fixture is now `live-history-2026-07-27.json` @ block 53950410; the offline `--fixture live-snapshot` path reports the same 7 moves the dashboard shows. Re-run whenever live history grows.
